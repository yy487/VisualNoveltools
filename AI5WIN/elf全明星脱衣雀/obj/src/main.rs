use ai5win_obj::encoding::EncodingRoute;
use ai5win_obj::workflow::{extract_path, inject_path, verify_path, WorkflowStats};
use ai5win_obj::{fail, Result};
use std::collections::HashMap;
use std::env;
use std::io::{self, Write};
use std::path::{Path, PathBuf};

const HELP: &str = r#"ai5win-obj-tool - AI5WIN Mahjong OBJ text extractor/injector

USAGE:
  ai5win-obj-tool extract --input <FILE_OR_DIR> --output <DIR> [--encoding cp932] [--overwrite]
  ai5win-obj-tool inject --source <FILE_OR_DIR> --translation <DIR> --output <FILE_OR_DIR> --encoding <cp932|gbk> [--overwrite]
  ai5win-obj-tool verify --input <FILE_OR_DIR> [--encoding cp932|gbk]
  ai5win-obj-tool [PATH]

NOTES:
  Input may contain plain OBJ bytecode or LZSS-compressed Data.Arc member blobs.
  JSON is always UTF-8. Edit only message; scr_msg and underscored fields are immutable.
  GBK injection re-encodes every extracted message, including unchanged messages.
"#;
...
fn print_stats(action: &str, stats: WorkflowStats) {
    println!(
        "{action}: {} OBJ files, {} messages ({} files scanned)",
...
fn interactive_extract(prefill: Option<PathBuf>) -> Result<()> {
    ...
    let input = PathBuf::from(prompt("Input OBJ file/directory", default_input)?);
    let output = PathBuf::from(prompt(
        "Translation output directory",
        Some("obj_translation_json"),
    )?);
...
fn interactive_inject(prefill: Option<PathBuf>) -> Result<()> {
    ...
    let source = PathBuf::from(prompt("Original OBJ file/directory", default_source)?);
