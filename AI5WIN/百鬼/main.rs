...
fn build_text_entry(
    ...
    let segments: Vec<TextSegment> = msg_indices
        ...
        .collect();
    let scr_msg = msg_indices
        .iter()
        .map(|&idx| strings[idx].text.as_str())
        .collect::<String>();
    let (name_inst_offset, name_offset, name_size, name_raw_hex, name) = if let Some(idx) = name_idx
    {
        let s = &strings[idx];
        (
            Some(s.start),
...
fn collect_text_entries(data: &[u8], source_name: &str) -> Result<Vec<TextEntry>, String> {
    ...
    let text_indices: Vec<usize> = strings
        .iter()
        .enumerate()
        .filter_map(|(idx, s)| is_translatable_text(&s.text).then_some(idx))
        .collect();
...
fn cstring_mask(data: &[u8], table_end: usize) -> Vec<bool> {
...
fn remap_target(
    ...
    if target < table_end || target >= data_len {
        return None;
    }
    Some(old_to_new[target])
}

fn relocate_vm_targets(
    out: &mut [u8],
    data: &[u8],
    old_to_new: &[usize],
    table_end: usize,
) -> Result<usize, String> {
    let mask = cstring_mask(data, table_end);
    let mut fixes = 0usize;

    for old_pos in table_end..data.len() {
        if mask.get(old_pos).copied().unwrap_or(false) {
            continue;
        }
        if old_pos + 3 <= data.len() && data[old_pos] == 0xf3 {
            let old_target = u16::from_le_bytes([data[old_pos + 1], data[old_pos + 2]]) as usize;
            if let Some(new_target) = remap_target(old_to_new, table_end, data.len(), old_target) {
                if new_target != old_target {
                    if new_target > 0xffff {
                        return Err(format!(
                            "16-bit VM target overflow at 0x{:x}: 0x{:x} -> 0x{:x}",
                            old_pos, old_target, new_target
                        ));
                    }
                    let new_pos = old_to_new[old_pos];
                    out[new_pos + 1..new_pos + 3]
                        .copy_from_slice(&(new_target as u16).to_le_bytes());
                    fixes += 1;
                }
            }
        }
        if old_pos + 6 <= data.len()
            && data[old_pos] == 0xef
            && data[old_pos + 1] == 0xff
            && !mask.get(old_pos + 1).copied().unwrap_or(false)
        {
            ...
            if let Some(new_target) = remap_target(old_to_new, table_end, data.len(), old_target) {
                if new_target != old_target {
                    let new_pos = old_to_new[old_pos];
                    out[new_pos + 2..new_pos + 6]
                        .copy_from_slice(&(new_target as u32).to_le_bytes());
                    fixes += 1;
                }
            }
        }
        if old_pos + 5 <= data.len() && data[old_pos] == 0x0a {
            let old_target = u32::from_le_bytes([
                data[old_pos + 1],
                data[old_pos + 2],
                data[old_pos + 3],
...
fn rebuild_inline(
    ...
    let vm_fixes = relocate_vm_targets(&mut out, data, &old_to_new, table_end)?;
    Ok((out, fixes, vm_fixes))
}
            ...
            for entry in entries {
                if let Some(segments) = &entry.message_segments {
                    ...
                    let new_parts = if entry.message == entry.scr_msg {
                        ...
                        split_message_for_segments(&entry.message, segments)
                    };
                    ...
                    let encoded = encode_text(&entry.message)?;
                    if encoded != old_raw {
