// TLG5/6 to PNG converter for KiriKiri images.
// Decoder core derived from GARbro ImageTLG.cs (W.Dee / morkt); standalone wrapper by Codex.
using System;
using System.IO;
...
sealed class BinReader : IDisposable
{
    readonly Stream _s;
    ...
    public void Seek(long off, SeekOrigin origin) => _s.Seek(off, origin);
    public byte ReadUInt8() { int v = _s.ReadByte(); if (v < 0) throw new EndOfStreamException(); return (byte)v; }
    public int ReadInt32() { Span<byte> b = stackalloc byte[4]; ReadExactly(b); return BitConverter.ToInt32(b); }
    public byte[] ReadBytes(int count) { byte[] b = new byte[count]; int got = _s.Read(b, 0, count); if (got != count) Array.Resize(ref b, got); return b; }
    public int Read(byte[] b, int off, int count) => _s.Read(b, off, count);
    void ReadExactly(Span<byte> b) { int off = 0; while (off < b.Length) { int n = _s.Read(b.Slice(off)); if (n <= 0) throw new EndOfStreamException(); off += n; } }
}

static class LittleEndian
{
    public static ushort ToUInt16(byte[] b, int o) => BitConverter.ToUInt16(b, o);
    public static uint ToUInt32(byte[] b, int o) => BitConverter.ToUInt32(b, o);
    public static int ToInt32(byte[] b, int o) => BitConverter.ToInt32(b, o);
}

static class ByteArrayExt
{
    public static bool AsciiEqual(this byte[] b, string s) => AsciiEqual(b, 0, s);
    public static bool AsciiEqual(this byte[] b, int o, string s)
    {
        if (o < 0 || o + s.Length > b.Length) return false;
        for (int i = 0; i < s.Length; i++) if (b[o+i] != (byte)s[i]) return false;
        return true;
    }
    public static uint ToUInt32(this byte[] b, int o) => BitConverter.ToUInt32(b, o);
}

sealed class TlgDecoder
{
    public TlgMetaData ReadMetaData(BinReader stream)
    {
        long pos = stream.Position;
        stream.Position = 0;
        var header = stream.ReadBytes(0x26);
        stream.Position = pos;
        if (header.Length < 0x26) return null;
        int offset = 0xf;
        if (!header.AsciiEqual("TLG0.0\x00sds\x1a")) offset = 0;
        int version;
        if (!header.AsciiEqual(offset+6, "\x00raw\x1a")) return null;
        if (0xAB == header[offset]) header[offset] = (byte)'T';
        if (header.AsciiEqual(offset, "TLG6.0")) version = 6;
        else if (header.AsciiEqual(offset, "TLG5.0")) version = 5;
        else if (header.AsciiEqual(offset, "XXXYYY")) { version = 5; header[offset+0x0C] ^= 0xAB; header[offset+0x10] ^= 0xAC; }
        else if (header.AsciiEqual(offset, "XXXZZZ")) { version = 6; header[offset+0x0F] ^= 0xAB; header[offset+0x13] ^= 0xAC; }
        else if (header.AsciiEqual(offset, "JKMXE8")) { version = 5; header[offset+0x0C] ^= 0x1A; header[offset+0x10] ^= 0x1C; }
        else return null;
        int colors = header[offset+11];
        if (6 == version)
        {
            if (1 != colors && 4 != colors && 3 != colors) return null;
            if (header[offset+12] != 0 || header[offset+13] != 0 || header[offset+14] != 0) return null;
            offset += 15;
        }
        else
        {
        ...
        return new TlgMetaData { Width = header.ToUInt32(offset), Height = header.ToUInt32(offset+4), BPP = colors*8, Version = version, DataOffset = offset+8 };
    }

    public byte[] Decode(BinReader src, TlgMetaData info)
    {
        src.Position = info.DataOffset;
        return info.Version == 6 ? ReadV6(src, info) : ReadV5(src, info);
    }
        ...
        byte[] ReadV6 (BinReader src, TlgMetaData info)
        {
            ...
            for (int y = 0; y < height; y += TVP_TLG6_H_BLOCK_SIZE)
            {
                ...
                for (int c = 0; c < colors; c++)
                {
                    ...
                    case 0:
                        ...
                        break;
                    default:
                        throw new InvalidDataException ("Unsupported entropy coding method");
                    }
                }
        ...
        byte[] ReadV5 (BinReader src, TlgMetaData info)
        {
            int width = (int)info.Width;
            int height = (int)info.Height;
            int colors = info.BPP / 8;
...
static class PngWriter
{
    ...
    {
        var t = new uint[256];
        for (uint n = 0; n < t.Length; n++) { uint c = n; for (int k = 0; k < 8; k++) c = ((c & 1) != 0) ? 0xedb88320u ^ (c >> 1) : c >> 1; t[n] = c; }
        return t;
    }
    static void U32(Stream s, uint v) { s.WriteByte((byte)(v>>24)); s.WriteByte((byte)(v>>16)); s.WriteByte((byte)(v>>8)); s.WriteByte((byte)v); }
    ...
    static void Chunk(Stream s, string type, byte[] data)
    {
        ...
        U32(s, (uint)data.Length); s.Write(tb); s.Write(data); U32(s, Crc(tb, data));
    }
    public static void Write(string path, int width, int height, byte[] bgra)
    {
        ...
        Be(0,width); Be(4,height); ihdr[8]=8; ihdr[9]=6;
        Chunk(fs, "IHDR", ihdr);
        ...
        for (int y = 0; y < height; y++)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                byte b = bgra[src++], g = bgra[src++], r = bgra[src++], a = bgra[src++];
                raw.WriteByte(r); raw.WriteByte(g); raw.WriteByte(b); raw.WriteByte(a);
            }
        }
        using var comp = new MemoryStream();
        using (var ds = new ZLibStream(comp, CompressionLevel.Optimal, leaveOpen:true)) raw.WriteTo(ds);
...
static class Program
{
    static int Main(string[] args)
    {
        ...
        {
            Console.Error.WriteLine("usage: tlg2png <input.tlg> [output.png]");
            return 2;
        }
        string input = args[0];
        string output = args.Length == 2 ? args[1] : Path.ChangeExtension(input, ".png");
        using var br = new BinReader(File.OpenRead(input));
        var dec = new TlgDecoder();
        var meta = dec.ReadMetaData(br);
        if (meta == null) { Console.Error.WriteLine("not a supported TLG5/6 file"); return 1; }
        var pixels = dec.Decode(br, meta);
        if (pixels == null) { Console.Error.WriteLine("decode failed"); return 1; }
        PngWriter.Write(output, (int)meta.Width, (int)meta.Height, pixels);
        Console.WriteLine($"{input} -> {output} ({meta.Width}x{meta.Height}, TLG{meta.Version})");
        return 0;
    }
}
