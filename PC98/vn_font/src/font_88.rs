//! KANJI1 ROM backend extracted from Foxy. No game script parsing or output writes.
...
impl FontResources {
    ...
    /// Load a paired support/candidate JSON and FCG1 16x16 glyph table.
    pub fn from_bytes(mapping_json: &[u8], glyph_table: &[u8]) -> FontResult<Self> {
        let (supports, candidate_pool) = parse_support_and_candidates(mapping_json)?;
    ...
    /// text. Script parsing and control-token handling belong to the game adapter.
...
impl FontResources {
    ...
    pub fn validate_plan(&self, plan: &DynamicFontPlan) -> FontResult<()> {
        ...
        for entry in &plan.mapping_used {
            if native_cp932(entry.target)?.is_some() {
                return Err("a native CP932 character cannot be remapped".into());
            }
            let support = self
                .supports
                .get(&entry.target)
                .ok_or("font plan contains unsupported target")?;
            if !self.candidate_pool.iter().any(|c| {
                ...
                return Err("font plan contains a carrier outside this resource set".into());
            }
            let preferred = support.preferred_cp932 == Some(entry.carrier_cp932)
                && support.preferred_rom_address == Some(entry.rom_address);
            if preferred != entry.used_preferred_carrier {
                return Err("font-plan preferred-carrier flag is inconsistent".into());
            }
        }
        Ok(())
    }
