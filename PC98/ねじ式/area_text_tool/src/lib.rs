...
pub const JSON_FORMAT: &str = "nejishiki-area-text-v1";
pub const HEADER_SIZE: usize = 44;
pub const VISIBLE_LINE_BYTE_LIMIT: usize = 60;
...
pub fn parse_area(bytes: &[u8]) -> Result<AreaFile> {
    ...
    for &(offset_section, pool_section) in &AREA_CHOICE_TABLE_SECTIONS {
        if counts[offset_section] == 0 {
            if counts[pool_section] != 0 {
                return Err(format!(
                    "AREA auxiliary sections {offset_section}/{pool_section} have a pool without an offset table"
                ));
            }
            continue;
        }
        if SECTION_WIDTHS[offset_section] != 2 || SECTION_WIDTHS[pool_section] != 1 {
            return Err(format!(
                "AREA auxiliary sections {offset_section}/{pool_section} have unexpected element widths"
            ));
        }

        let offset_table = &sections[offset_section];
        let pool = &sections[pool_section];
        if pool.is_empty() {
            let mut choices = Vec::with_capacity(usize::from(counts[offset_section]));
            for index in 0..usize::from(counts[offset_section]) {
                let start = usize::from(read_u16(offset_table, index * 2)?);
                if start != 0 {
                    return Err(format!(
                        "AREA auxiliary sections {offset_section}/{pool_section} have nonzero offset 0x{start:X} into an empty pool"
                    ));
                }
        ...
        for index in 0..usize::from(counts[offset_section]) {
            let start = usize::from(read_u16(offset_table, index * 2)?);
            let tail = pool.get(start..).ok_or_else(|| {
                format!(
                    "AREA auxiliary sections {offset_section}/{pool_section} offset 0x{start:X} points outside the pool"
                )
            })?;
            let nul = tail.iter().position(|&byte| byte == 0).ok_or_else(|| {
                format!(
                    "AREA auxiliary sections {offset_section}/{pool_section} string at pool offset 0x{start:X} has no NUL terminator"
                )
            })?;
            let text = decode_cp932(&tail[..nul]).map_err(|error| {
                format!(
                    "AREA auxiliary sections {offset_section}/{pool_section} at pool offset 0x{start:X}: {error}"
                )
            })?;
            choices.push(ChoiceSlot {
                export: !text.is_empty(),
                text,
                pool_offset: start,
            });
        }
    ...
    for (index, &start) in offsets.iter().enumerate() {
        let slot = pool.get(start..).ok_or_else(|| {
            format!("AREA message slot {index} offset 0x{start:X} is outside the text pool")
        })?;
        let nul = slot
            .iter()
...
fn area_auxiliary_type(offset_section: usize) -> &'static str {
    match offset_section {
        10 => "person",
        12 => "place",
        14 => "topic",
        16 => "object",
...
impl AreaFile {
        ...
        for table in &self.choice_tables {
            for choice in &table.choices {
                ...
                entries.push(TranslationEntry {
                    ...
                    _size: size,
                    _type: area_auxiliary_type(table.offset_section).to_string(),
                    _encoding: "cp932".to_string(),
                    _policy: "relocate".to_string(),
    ...
    fn rebuild(&self, texts: &[String], choice_texts: &[String]) -> Result<Vec<u8>> {
        ...
        for table in &self.choice_tables {
            ...
            let mut choice_pool = sections[table.pool_section].clone();
            let mut offset_bytes = sections[table.offset_section].clone();
            let mut rebuilt_aliases: HashMap<(usize, String), u16> = HashMap::new();
            let mut local_index = 0usize;
            for (slot_index, choice) in table.choices.iter().enumerate() {
                if !choice.export {
                    continue;
                }
                ...
                let offset = if let Some(&offset) = rebuilt_aliases.get(&alias_key) {
                    offset
                } else {
                    if choice_pool.len() > usize::from(u16::MAX) {
                        return Err(format!(
                            "auxiliary pool offset for index {} exceeds u16",
                            choice_cursor + local_index - 1
                        ));
                    }
                    let offset = choice_pool.len() as u16;
                    let encoded = encode_cp932(text).map_err(|error| {
                        format!(
                            "auxiliary index {}: {error}",
                            choice_cursor + local_index - 1
                        )
                    })?;
                ...
                offset_bytes[slot_index * 2..slot_index * 2 + 2]
                    .copy_from_slice(&offset.to_le_bytes());
            }
            if choice_pool.len() > usize::from(u16::MAX) {
                return Err(format!(
                    "rebuilt auxiliary pool for sections {}/{} is {} bytes, exceeds u16 limit 65535",
        ...
        let mut pool = sections[21].clone();
        let mut offset_bytes = sections[20].clone();
        let mut rebuilt_aliases: HashMap<(usize, String), u16> = HashMap::new();
        for (index, (text, slot)) in texts.iter().zip(&self.slots).enumerate() {
            if text == &slot.text {
                continue;
            }
            let alias_key = (slot.pool_offset, text.to_string());
            let offset = if let Some(&offset) = rebuilt_aliases.get(&alias_key) {
                offset
            } else {
            ...
            offset_bytes[index * 2..index * 2 + 2].copy_from_slice(&offset.to_le_bytes());
        }
        if pool.len() > usize::from(u16::MAX) {
            return Err(format!(
...
pub fn extract_path(input: &Path, output_root: &Path, overwrite: bool) -> Result<ExtractReport> {
    ...
    if sources.is_empty() {
        return Err(format!(
            "no structurally valid AREA text source found in {}",
            input.display()
        ));
    }
...
fn split_logical_parts(text: &str) -> Result<LogicalParts> {
    let trimmed_start = text.find(|ch: char| !ch.is_whitespace());
    let trimmed_end = text
        .rfind(|ch: char| !ch.is_whitespace())
        .map(|offset| offset + text[offset..].chars().next().expect("last char").len_utf8());
    if let (Some(start), Some(end)) = (trimmed_start, trimmed_end) {
        let trimmed = &text[start..end];
        let recovered_kind = if !trimmed.contains(['「', '『']) && trimmed.ends_with('」') {
            Some(PartKind::Dialogue)
        } else if !trimmed.contains(['「', '『']) && trimmed.ends_with('』') {
            Some(PartKind::Monologue)
        } else {
            None
        };
        if let Some(kind) = recovered_kind {
            return Ok(LogicalParts {
                spans: vec![PartSpan { start, end, kind }],
                warnings: vec![
                    "recovered a suffix-referenced string ending in an unmatched closing quote"
                        .to_string(),
                ],
            });
        }
...
fn parse_supported_source(bytes: &[u8]) -> Option<ParsedSource> {
    parse_area(bytes).ok().map(ParsedSource::Area)
}
...
mod tests {
    ...
    fn leaves_runtime_soft_wrapping_to_the_game() {
        ...
        assert!(visible_cp932_len(&message).expect("message width") > 60);
        assert_eq!(prepare_message(&message).expect("prepare"), message);
    }

    #[test]
    fn preserves_ordinary_symbols_and_explicit_hard_line_breaks() {
        let message = "○「みじかい。\n　つぎ。」△";
        assert_eq!(prepare_message(message).expect("prepare"), message);
    }
    ...
    #[test]
    fn changed_message_appends_without_rewriting_original_pool() {
        ...
        let pool_start = area.text_pool_file_offset;
        let pool_end = pool_start + area.sections[21].len();
        assert_eq!(&rebuilt[pool_start..pool_end], area.sections[21].as_slice());
        let reparsed = parse_area(&rebuilt).expect("reparse");
        assert!(reparsed.slots[0].pool_offset >= area.sections[21].len());
        assert_eq!(reparsed.slots[1].pool_offset, area.slots[1].pool_offset);
        assert_eq!(
    ...
    fn extracts_and_rebuilds_all_area_choice_tables() {
        ...
        let extracted_choices: Vec<_> = document
            ...
            .collect();
        assert_eq!(extracted_choices, ["十", "十二", "十四", "十六", "十八"]);
        let extracted_types: Vec<_> = document
            .entries
            .iter()
            .filter(|entry| entry._choice_index.is_some())
            .map(|entry| entry._type.as_str())
            .collect();
        assert_eq!(
            extracted_types,
            ["person", "place", "topic", "object", "item"]
        );
    ...
    #[test]
    fn accepts_line_break_in_auxiliary_text() {
        ...
        let (rebuilt, patched, unchanged) = area
            .rebuild_with_document(&document)
            .expect("auxiliary LF should be editable");
        assert_eq!(patched, 1);
        assert_eq!(unchanged, 1);
        assert_eq!(
            parse_area(&rebuilt).expect("reparse").choice_tables[0].choices[0].text,
            "改\n行"
        );
    }

    #[test]
    fn rejects_unencodable_text_but_accepts_ordinary_circle_triangle_symbols() {
    ...
    fn recursive_extract_flattens_json_and_disambiguates_duplicate_basenames() {
        ...
        fs::write(
            source.join("disk1").join("SAME.DAT"),
            make_area(&[("「一」", &[])]),
        )
        .expect("write first AREA");
        fs::write(
            source.join("disk2").join("SAME.DAT"),
            make_area(&[("「二」", &[])]),
        )
        .expect("write second AREA");
        fs::write(source.join("UNIQUE.DAT"), make_area(&[("「三」", &[])]))
            .expect("write unique AREA");
        ...
        assert!(output.join("UNIQUE.DAT.json").is_file());
        let duplicate_names: HashSet<_> = files
            .iter()
            .filter_map(|path| path.file_name().and_then(|name| name.to_str()))
            .filter(|name| name.ends_with("SAME.DAT.json"))
            .collect();
