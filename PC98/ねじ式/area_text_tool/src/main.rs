use nejishiki_area_text::{extract_path, inject_path};
use std::env;
use std::ffi::OsString;
use std::io::{self, Write};
use std::path::{Path, PathBuf};

const HELP: &str = r#"area_text 0.1.0

《ねじ式》结构化 AREA 文本提取/注入工具。
...
规则:
  ...
  只接受结构完整的 AREA 文件；正文与五类辅助文本均不输出 name。
  同一逻辑条目中的原作物理换行会移除；注入不主动折行，由游戏按 60 个半角字节自动换行。
