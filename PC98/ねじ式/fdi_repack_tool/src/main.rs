...
const HELP: &str = r#"fdi_pack 0.1.0
...
限制:
  ...
  FAT 副本原本不一致时，以第一份 FAT 工作并逐字节保留备用 FAT；报告会明确提示。
"#;
...
fn run_recursive_pack(
    ...
    for item in &report.outputs {
        println!(
            "[pack:image] source_archive={} packed_files={} modified_files={} reallocated_files={} fat_copies_identical={} fat_mismatch_preserved={} output_bytes={} output={}",
            item.source_archive,
            item.packed_files,
            item.modified_files,
            item.reallocated_files,
            item.fat_copies_identical,
            item.fat_mismatch_preserved,
            item.output_bytes,
            item.output.display()
        );
    }
...
fn run_single_pack(
    ...
    println!("[pack] reallocated_files={}", report.reallocated_files);
    println!(
        "[pack] fat_copies_identical={}",
        report.fat_copies_identical
    );
    println!(
        "[pack] fat_mismatch_preserved={}",
        report.fat_mismatch_preserved
    );
