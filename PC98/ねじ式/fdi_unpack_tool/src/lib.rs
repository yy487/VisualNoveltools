...
fn parse_fdi(path: &Path, bytes: &[u8]) -> Result<ParsedDisk> {
    ...
    for copy_index in 1..usize::from(fat_copies) {
        ...
        if bytes[copy_offset..copy_end] != *fat {
...
fn write_workspace(
    ...
    let write_result = (|| -> Result<UnpackReport> {
        ...
        Ok(UnpackReport {
            ...
            warnings: prepared
                .iter()
                .filter(|disk| !disk.parsed.fat12.fat_copies_identical)
                .map(|disk| {
                    format!(
                        "{}: FAT 副本不一致；已使用第一份 FAT，完整目录树、文件大小、簇链循环和交叉链接校验均已通过",
...
fn decode_short_name(entry: &[u8; 32]) -> Result<(String, String)> {
    ...
    let (stem_encoded, _, stem_errors) = SHIFT_JIS.encode(&stem);
    let (extension_encoded, _, extension_errors) = SHIFT_JIS.encode(&extension);
    if stem_errors
        || extension_errors
        || stem_encoded.as_ref() != stem_raw
        || extension_encoded.as_ref() != extension_raw
    {
        return Err(format!("目录项短文件名无法 CP932 字节往返: {raw_hex}"));
    }
...
mod tests {
    ...
    #[test]
    fn accepts_mismatched_backup_fat_and_records_warning_state() {
        let mut bytes = sample_fdi();
        bytes[HEADER_SIZE + SECTOR_SIZE * 2 + 10] ^= 1;
        let parsed = parse_fdi(Path::new("disk.fdi"), &bytes).expect("primary FAT is valid");
