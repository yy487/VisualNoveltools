...
fn build_image(
    ...
    validate_rebuilt(&rebuilt, &layout, archive, &prepared)?;
...
fn collect_source_images(input: &Path) -> Result<Vec<SourceImage>> {
    ...
    for path in paths {
        let bytes =
            fs::read(&path).map_err(|e| format!("无法读取原始镜像候选 {}: {e}", path.display()))?;
...
fn validate_manifest_archive(
    ...
    if !archive.fat12.fat_copies_identical {
        return Err("清单未确认 FAT 副本一致，拒绝封盘".to_string());
    }
...
fn validate_rebuilt(
    ...
    if reparsed.fdi != layout.fdi || reparsed.fat12 != layout.fat12 {
    ...
    for copy in 1..usize::from(layout.fat12.fat_copies) {
        ...
        if output[start..start + layout.fat_bytes] != *fat {
...
fn parse_layout(path: &Path, bytes: &[u8]) -> Result<DiskLayout> {
    ...
    for copy in 1..usize::from(fat_copies) {
        ...
        if end > bytes.len() || bytes[start..end] != *fat {
            return Err(format!("{}: FAT 副本不一致", path.display()));
        }
    }
    let fat12 = Fat12Info {
        bytes_per_sector,
