...
# -*- coding: utf-8 -*-
"""
Meguri / Mizuka-style .p archive unpacker/repacker with per-file payload decode.

Validated with the supplied se.p sample:
  magic       pipi
  version     13
  seed        41
  file_count  159
...
Version 2 also mirrors the runtime open/read split: for v10-v13 entries
...
def parse_archive(path: Path, exe_path: Path | None = None) -> Archive:
    ...
    for idx in range(file_count):
        if version >= 10:
            ...
            raise NotImplementedError("this implementation currently supports v10-v13 table entries")
...
def build_table_and_payload(manifest: dict[str, Any], files_dir: Path) -> tuple[bytes, bytes, int]:
    ...
    table_len = sum(2 + n for n in body_lengths)
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="Unpack/repack Meguri/Mizuka .p archives")
